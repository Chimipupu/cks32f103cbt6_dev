...
static void move_cursor_right(void)
{
    if (s_cursor_pos < s_cmd_index) {
        Serial.printf("%c", s_cmd_buffer[s_cursor_pos]);
        s_cursor_pos++;
    }
}
...
static void delete_char_at_cursor(void)
{
    if (s_cursor_pos < s_cmd_index) {
        ...
        for (int32_t i = s_cursor_pos; i < s_cmd_index; i++) {
            Serial.printf("%c", s_cmd_buffer[i]);
        }
        Serial.printf(" ");  // 最後の文字を消去
...
static int32_t split_str(char* p_str, dbg_cmd_args_t *p_args)
{
    ...
    p_args->argc = 0;
#ifdef DEBUG_DBG_COM
    Serial.printf("[DEBUG] : Input string = '%s'\n", p_str);  // 入力文字列の確認
#endif // DEBUG_DBG_COM
...
#ifdef DEBUG_DBG_COM
        Serial.printf("[DEBUG] : Next token = '%s'\n", p_token);
#endif // DEBUG_DBG_COM
...
#ifdef DEBUG_DBG_COM
    Serial.printf("[DEBUG] : argc = %d\n", p_args->argc);
    for (int i = 0; i < p_args->argc; i++)
    {
        Serial.printf("[DEBUG] : argv[%d] = %s\n", i, p_args->p_argv[i]);
    }
#endif // DEBUG_DBG_COM
...
static dbg_cmd_t dbg_com_parse_cmd(const char* p_cmd_str, dbg_cmd_args_t *p_args)
{
    for (uint8_t i = 0; i < g_cmd_tbl_size; i++)
    {
        if (strcmp(p_cmd_str, g_cmd_tbl[i].p_cmd_str) == 0) {
            ...
            //     Serial.printf("Error: Invalid number of arguments. Expected %d-%d, got %d\n",
